...
bool LaneBorrowPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  for (size_t i = 0; i < decided_side_pass_direction_.size(); i++) {
    ...
    auto front_static_obstacle_id = injector_->planning_context()
        ->planning_status().path_decider().front_static_obstacle_id();
    if (front_static_obstacle_id == "5364") {
      PathBoundsDeciderUtil::ExtendBoundaryByADC(
          *reference_line_info_, init_sl_state_, 1.5,
          &path_bound);
    }
...
bool LaneBorrowPath::OptimizePath(
  ...
  if (is_dynamic_blocking_obstacle_) {
    config = config_.dynamic_path_optimizer_config();
  }
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    if (decided_side_pass_direction_.empty()) {
      ...
      CheckLaneBorrow(*reference_line_info_, &left_borrowable,
                      &right_borrowable);
      if (mutable_path_decider_status->front_static_obstacle_id() == "5364") {
...
bool LaneBorrowPath::IsBlockingObstacleWithinDestination(
  ...
  is_dynamic_blocking_obstacle_ = false;
  if (blocking_obstacle->speed() > 0.5) {
    is_dynamic_blocking_obstacle_ = true;
  }
...
bool LaneBorrowPath::IsBlockingObstacleFarFromIntersection(
  ...
  if (path_decider_status.front_static_obstacle_id() == "5364") {
    return true;
  }
