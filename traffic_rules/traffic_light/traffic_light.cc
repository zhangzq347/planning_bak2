...
Status TrafficLight::ApplyRule(Frame* const frame,
  ...
  for (const auto& junction : map_path.junction_overlaps()) {
    reference_line->AddSpeedLimit(junction.start_s - 3, junction.end_s + 3, 5.8);
  }
  for (const auto& crosswalk : map_path.crosswalk_overlaps()) {
    reference_line->AddSpeedLimit(crosswalk.start_s - 3, crosswalk.end_s + 3, 4.8);
  }
...
void TrafficLight::MakeDecisions(Frame* const frame,
  ...
  for (const auto& traffic_light_overlap : traffic_light_overlaps) {
    if (traffic_light_overlap.object_id == left_turn_id &&
        traffic_light_overlap.start_s > start_left_s) {
